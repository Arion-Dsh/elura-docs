...
```text
 ...
 客户端 ── ELR2/TCP 或 WebSocket ──> Gateway ── ELR2/TCP ──> World
                                      │   │                     │
                                      │   ├── 服务发现          ├── 处理器
                                      │   ├── 会话状态          ├── 中间件
                                      │   └── 准入控制          └── 持久化
...
`WorldLauncher` 提供 `with_push_transport`、`with_registrar` 等聚焦方法，
`WorldBuilder` 则负责路由和中间件。Launcher 不会猜测应该使用哪个 Provider。

## 单体与拆分运行时

`MonolithLauncher` 保留 Gateway 和 World 的概念，但在进程内连接两者。它消除了
